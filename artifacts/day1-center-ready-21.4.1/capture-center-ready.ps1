...
foreach ($viewport in @(@{ width = 320; height = 800 }, @{ width = 360; height = 800 }, @{ width = 390; height = 844 }, @{ width = 412; height = 786 }, @{ width = 430; height = 932 })) {
  ...
  $path = "C:\Users\user\Downloads\center-ready-$($viewport.width)x$($viewport.height).png"
  [IO.File]::WriteAllBytes($path, [Convert]::FromBase64String($shot.result.data))
  $expression = "JSON.stringify((()=>{const d=document.documentElement,b=document.querySelector('.center-ready-action button').getBoundingClientRect(),o=document.querySelector('.center-ready-optical').getBoundingClientRect(),n=document.querySelector('.center-ready-note').getBoundingClientRect();return{viewport:[innerWidth,innerHeight],clientWidth:d.clientWidth,scrollWidth:d.scrollWidth,documentHeight:d.scrollHeight,overflow:d.scrollWidth-d.clientWidth,button:{top:b.top,bottom:b.bottom},optical:{left:o.left,right:o.right,top:o.top,bottom:o.bottom},note:{top:n.top,bottom:n.bottom}}})())"
