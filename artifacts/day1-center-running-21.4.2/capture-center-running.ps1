...
foreach ($viewport in @(@{ width = 320; height = 800 }, @{ width = 360; height = 800 }, @{ width = 390; height = 844 }, @{ width = 412; height = 786 }, @{ width = 430; height = 932 })) {
  ...
  $path = "C:\Users\user\Downloads\center-running-$($viewport.width)x$($viewport.height).png"
  [IO.File]::WriteAllBytes($path, [Convert]::FromBase64String($shot.result.data))
  $expression = "JSON.stringify((()=>{const d=document.documentElement,s=document.querySelector('.center-shape').getBoundingClientRect(),n=document.querySelector('.center-running-note').getBoundingClientRect(),x=getComputedStyle(document.querySelector('.center-running-decoration')).pointerEvents;return{viewport:[innerWidth,innerHeight],clientWidth:d.clientWidth,scrollWidth:d.scrollWidth,documentHeight:d.scrollHeight,overflow:d.scrollWidth-d.clientWidth,surface:{left:s.left,right:s.right,top:s.top,bottom:s.bottom,width:s.width,height:s.height},note:{top:n.top,bottom:n.bottom},decorationPointerEvents:x,markers:document.querySelectorAll('.center-running-poster .center-marker,.center-running-poster .center-crosshair').length}})())"
