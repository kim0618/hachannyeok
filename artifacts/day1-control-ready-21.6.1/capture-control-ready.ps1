...
Eval "localStorage.clear();location.reload()"; Start-Sleep -Milliseconds 700
...
foreach($viewport in @(@{width=320;height=800},@{width=360;height=800},@{width=390;height=844},@{width=412;height=786},@{width=430;height=932})){
  ...
  $shot=Send-Cdp 'Page.captureScreenshot' @{format='png';fromSurface=$true;captureBeyondViewport=$false};[IO.File]::WriteAllBytes("C:\Users\user\Downloads\control-ready-$($viewport.width)x$($viewport.height).png",[Convert]::FromBase64String($shot.result.data))
  $expression="JSON.stringify((()=>{const d=document.documentElement,p=document.querySelector('.control-ready-preview'),n=document.querySelector('.control-ready-note'),b=document.querySelector('.control-ready-action button'),pr=p.getBoundingClientRect(),nr=n.getBoundingClientRect(),br=b.getBoundingClientRect();return{viewport:[innerWidth,innerHeight],overflow:d.scrollWidth-d.clientWidth,documentHeight:d.scrollHeight,preview:{left:pr.left,right:pr.right,top:pr.top,bottom:pr.bottom,pointerEvents:getComputedStyle(p).pointerEvents},note:{top:nr.top,bottom:nr.bottom},button:{top:br.top,bottom:br.bottom},runningTrack:document.querySelectorAll('.control-track').length,readyCount:document.querySelectorAll('.control-ready-content').length}})())";$metrics=Send-Cdp 'Runtime.evaluate' @{expression=$expression;returnByValue=$true};Write-Output "$($viewport.width): $($metrics.result.result.value)"
}
$socket.Dispose()
