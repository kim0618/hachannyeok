...
foreach ($viewport in @(@{ width = 320; height = 800 }, @{ width = 360; height = 800 }, @{ width = 390; height = 844 }, @{ width = 412; height = 786 }, @{ width = 430; height = 932 }, @{ width = 1280; height = 1200 })) {
  ...
  $path = "C:\Users\jinsung\Downloads\day1-time-ready-21.3.1-$($viewport.width)-exact.png"
  [IO.File]::WriteAllBytes($path, [Convert]::FromBase64String($shot.result.data))
  $expression = "JSON.stringify((()=>{const d=document.documentElement,c=document.querySelector('.time-ready-reference-start').getBoundingClientRect(),p=document.querySelector('.time-ready-reference-poster img').getBoundingClientRect();return{viewport:[innerWidth,innerHeight],clientWidth:d.clientWidth,scrollWidth:d.scrollWidth,documentHeight:d.scrollHeight,overflow:d.scrollWidth-d.clientWidth,start:{x:c.x,y:c.y,width:c.width,height:c.height,bottom:c.bottom},poster:{x:p.x,y:p.y,width:p.width,height:p.height,bottom:p.bottom}}})())"
