...
foreach ($viewport in @(@{ width = 360; height = 800 }, @{ width = 412; height = 786 })) {
  ...
  $path = "C:\Users\user\Downloads\time-running-$($viewport.width)x$($viewport.height).png"
  [IO.File]::WriteAllBytes($path, [Convert]::FromBase64String($shot.result.data))
  $expression = "JSON.stringify((()=>{const d=document.documentElement,b=document.querySelector('.time-running-action button').getBoundingClientRect(),i=document.querySelector('.time-running-instrument').getBoundingClientRect();return{viewport:[innerWidth,innerHeight],clientWidth:d.clientWidth,scrollWidth:d.scrollWidth,documentHeight:d.scrollHeight,overflow:d.scrollWidth-d.clientWidth,button:{left:b.left,right:b.right,top:b.top,bottom:b.bottom},instrument:{left:i.left,right:i.right,top:i.top,bottom:i.bottom}}})())"
