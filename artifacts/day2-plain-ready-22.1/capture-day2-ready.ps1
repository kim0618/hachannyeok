...
Send-Cdp 'Page.navigate' @{ url = 'http://127.0.0.1:5174/artifacts/day2-plain-ready-22.1/capture.html' } | Out-Null
Start-Sleep -Milliseconds 800
Send-Cdp 'Runtime.evaluate' @{ expression = "document.querySelector('.primary-button').click()" } | Out-Null
Start-Sleep -Milliseconds 300
$out = 'C:\Users\user\Downloads\day2-plain-ready-22.1'
...
foreach ($viewport in @(@{ width = 360; height = 800 }, @{ width = 412; height = 786 })) {
  ...
  [IO.File]::WriteAllBytes("$out\day2-plain-ready-$($viewport.width)x$($viewport.height).png", [Convert]::FromBase64String($shot.result.data))
  $expression = "JSON.stringify((()=>{const d=document.documentElement,b=document.querySelector('.day2-ready-action .primary-button').getBoundingClientRect(),i=document.querySelector('.day2-ready-dial').getBoundingClientRect();return{viewport:[innerWidth,innerHeight],clientWidth:d.clientWidth,scrollWidth:d.scrollWidth,documentHeight:d.scrollHeight,overflow:d.scrollWidth-d.clientWidth,cta:{left:b.left,right:b.right,width:b.width,bottom:b.bottom},dial:{left:i.left,right:i.right,width:i.width}}})())"
