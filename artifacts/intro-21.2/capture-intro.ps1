...
Send-Cdp 'Page.navigate' @{ url = 'http://127.0.0.1:5174/' } | Out-Null
Start-Sleep -Milliseconds 900
Send-Cdp 'Runtime.evaluate' @{ expression = "document.querySelector('.home-reference-cta').click()" } | Out-Null
Start-Sleep -Milliseconds 300

foreach ($viewport in @(@{ width = 360; height = 800 }, @{ width = 412; height = 786 }, @{ width = 430; height = 932 }, @{ width = 1280; height = 1200 })) {
  ...
  $path = "C:\Users\jinsung\Downloads\intro-21.2-$($viewport.width)-exact.png"
  [IO.File]::WriteAllBytes($path, [Convert]::FromBase64String($shot.result.data))
  $expression = "JSON.stringify((()=>{const d=document.documentElement,c=document.querySelector('.intro-reference-start').getBoundingClientRect(),b=document.querySelector('.intro-reference-back').getBoundingClientRect(),p=document.querySelector('.intro-reference-poster img').getBoundingClientRect();return{viewport:[innerWidth,innerHeight],clientWidth:d.clientWidth,scrollWidth:d.scrollWidth,documentHeight:d.scrollHeight,overflow:d.scrollWidth-d.clientWidth,start:{x:c.x,y:c.y,width:c.width,height:c.height,bottom:c.bottom},back:{x:b.x,y:b.y,width:b.width,height:b.height,bottom:b.bottom},poster:{x:p.x,y:p.y,width:p.width,height:p.height,bottom:p.bottom}}})())"
