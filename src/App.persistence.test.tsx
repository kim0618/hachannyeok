...
describe('App persistence startup', () => {
  it('load 전 최초 사용자 화면을 flash하지 않고 empty load 뒤 STATE A를 표시한다', async () => {
    ...
    expect(screen.queryByRole('heading', { name: '하찮력' })).not.toBeInTheDocument();
    expect(await screen.findByRole('heading', { name: '하찮력' })).toBeInTheDocument();
  });
  ...
  it('확인 후 clear하면 memory와 storage가 STATE A로 초기화된다', async () => {
    ...
    expect(screen.getByRole('heading', { name: '하찮력' })).toBeInTheDocument();
  });
  ...
  it('DAY 2 다음 유효 날짜에는 DAY 3 준비 화면만 연결하고 날짜 역행은 unlock하지 않는다', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '오늘의 추가 분석' })); expect(screen.getByRole('heading', { name: /다음 추가 분석은/ })).toBeInTheDocument(); next.unmount();
