...
export const SPATIAL_MEMORY_PERMUTATIONS = [[0,1,2],[0,2,1],[1,0,2],[1,2,0],[2,0,1],[2,1,0]] as const;
export function matchSpatialMemoryPositions(trial: SpatialMemoryTrial): { permutation: readonly [number,number,number]; totalDistance: number; meanDistance: number } | null { if (!trial.valid) return null; let best:{permutation:readonly[number,number,number];totalDistance:number}|null=null;for(const order of SPATIAL_MEMORY_PERMUTATIONS){const totalDistance=order.reduce<number>((sum,selectedIndex,shownIndex)=>sum+euclideanDistance(trial.shownPositions[shownIndex]!,trial.selectedPositions[selectedIndex]!),0);if(!best||totalDistance<best.totalDistance)best={permutation:order,totalDistance};}return best?{...best,meanDistance:best.totalDistance/3}:null; }
export function matchingMeanDistance(trial: SpatialMemoryTrial): number | null { return matchSpatialMemoryPositions(trial)?.meanDistance ?? null; }
