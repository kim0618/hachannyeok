...
export function BasicAnalysisScreen({ baseline, analysis, dailyCount = 0, onRestart, onHome, saveStatus = 'saved', onRetrySave, sharePort = appsInTossShare }: Props) {
  ...
  return <div className="screen analysis-screen">
    ...
    <main>
      ...
      <div className="analysis-actions"><ShareAction message={basicShareMessage(value)} sharePort={sharePort} statusId="share-status"/><button className="secondary-button" type="button" disabled aria-describedby="detail-status">상세 분석 보기</button><p id="detail-status">상세 분석은 준비 중입니다.</p>{onHome && <button className="secondary-button" type="button" onClick={onHome}>홈으로</button>}</div>
    </main>
  </div>;
}
