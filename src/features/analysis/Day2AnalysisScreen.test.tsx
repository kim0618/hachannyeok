...
function renderResult(record: DailyRecord) {
  ...
  render(<Day2AnalysisScreen record={record} before={before.value} after={after.value} saveStatus="saved" onHome={() => undefined} />);
...
describe('Day2AnalysisScreen', () => {
  ...
  it('eligible tendency의 late 방향과 baseline score 변화를 표시한다', () => {
    ...
    expect(screen.getByRole('heading', { name: /시간이 조금 늦어지는 편/ })).toBeInTheDocument();
  ...
  it('eligible degradation과 neutral signed direction을 함께 보존한다', () => {
    ...
    expect(screen.getByRole('heading', { name: '방해가 있으면 오차 폭이 커지는 편' })).toBeInTheDocument();
  ...
  it('eligible tendency의 earlier 방향을 유지한다', () => {
    ...
    expect(screen.getByRole('heading', { name: /시간이 조금 빨라지는 편/ })).toBeInTheDocument();
  ...
  it('threshold 미달이면 fallback과 점수 유지 문구를 표시한다', () => {
    ...
    expect(screen.getByRole('heading', { name: /거의 그대로/ })).toBeInTheDocument();
  ...
  it('작은 조건 차이를 ms 비교 카드로 보여주고 점수는 보조 위계에 둔다', () => {
