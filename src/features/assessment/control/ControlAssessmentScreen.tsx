...
export function ControlAssessmentScreen(props: Props) {
  ...
  return <div className="screen control-screen ready-screen"><header className="progress-header"><span>손가락 통제</span><span>4 / 5</span></header><section className="ready-content" aria-labelledby="control-ready-title"><p className="eyebrow">네 번째 측정</p><h1 id="control-ready-title">손가락 통제</h1><p>움직이는 표시가 목표 지점에 왔다고 느껴질 때 멈춰주세요.<br />눈으로 보고 바로 눌러보세요.</p></section><div className="bottom-action"><PrimaryButton onClick={assessment.startTrial}>시작하기</PrimaryButton></div></div>;
}
