...
describe('Day5ControlAssessmentScreen signature', () => {
  it('RUNNING은 condition identity 없이 cue 없는 중립 frame을 표시한다', () => {
    ...
    expect(screen.getByRole('button',{name:'지금 멈추기'})).toBeEnabled();
    expect(screen.getByText('1 / 4')).toBeInTheDocument();
    expect([...view.container.querySelectorAll('.day5-condition-progress strong')].map((node) => node.textContent)).toEqual(['INTRO','01','02','03','04','ANALYSIS']);
    expect(view.container).not.toHaveTextContent(/\bP1\b|\bS1\b|\bP2\b|\bS2\b/);
    expect(view.container.querySelector('.day5-condition-card')).not.toBeInTheDocument();
  ...
  it('READY는 변화 가능성을 안내하고 RESULT는 실제 condition을 공개하지만 다음 RUNNING은 중립이다',()=>{let now=0;const view=render(<Day5ControlAssessmentScreen sessionDateKey="2026-08-16" dateNow={()=>new Date('2026-08-16T12:00:00Z')} clock={{now:()=>now}} animationScheduler={scheduler} createTrialId={()=> 'signature-day5-surprise'} onDateInvalidated={vi.fn()} onComplete={vi.fn()}/>);expect(screen.getByText('움직임이 중간에 달라질 수 있어요.')).toBeVisible();fireEvent.click(screen.getByRole('button',{name:'측정 시작'}));now=500;fireEvent.click(screen.getByRole('button',{name:'지금 멈추기'}));expect(screen.getByText('예측 가능한 움직임 기록입니다.')).toBeVisible();fireEvent.click(screen.getByRole('button',{name:'다음 측정'}));expect(screen.getByText('움직임 통제 측정')).toBeVisible();expect(screen.getByText('2 / 4')).toBeInTheDocument();expect(view.container).not.toHaveTextContent(/\bP1\b|\bS1\b|\bP2\b|\bS2\b/);expect(view.container).not.toHaveTextContent(/SURPRISE|PREDICTABLE|STEADY MOTION|VARIABLE MOTION|변속 조건|예측 조건/);expect(document.querySelector('.speed-cue,.transition-warning,.proximity-cue')).not.toBeInTheDocument();});
});
