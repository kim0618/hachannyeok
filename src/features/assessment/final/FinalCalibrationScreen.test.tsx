...
describe('FinalCalibrationScreen',()=>{
  ...
  it('memory exposure에서 backgrounded 된 뒤 stale timer가 recall을 열지 않는다',()=>{
    ...
    fireEvent.click(screen.getByRole('button',{name:'최종 보정 시작'}));act(()=>frames.run(0));act(()=>frames.run(1));fireEvent.click(screen.getByRole('button',{name:'focus-baseline-2-item-08'}));fireEvent.click(screen.getByRole('button',{name:'다음 측정'}));
  ...
  it('control은 exact end에서 invalid이고 end 직전 stop은 valid다',()=>{
    ...
    fireEvent.click(screen.getByRole('button',{name:'최종 보정 시작'}));now=2624.999;fireEvent.click(screen.getByRole('button',{name:'멈춰!'}));expect(screen.getByText('이번 측정이 기록됐습니다.')).toBeInTheDocument();first.unmount();
    ...
    fireEvent.click(screen.getByRole('button',{name:'최종 보정 시작'}));now=2625;fireEvent.click(screen.getByRole('button',{name:'멈춰!'}));expect(screen.getByText('이번 측정은 제외됩니다.')).toBeInTheDocument();
  });
  it('control RAF가 먼저 exact end를 관찰해도 한 번만 invalid 처리한다',()=>{
    ...
    fireEvent.click(screen.getByRole('button',{name:'최종 보정 시작'}));now=2625;act(()=>frames.run(0));
  ...
  it('3등분 divider crossing 입력을 거부하고 valid raw 결과를 만든다',()=>{
    ...
    fireEvent.click(screen.getByRole('button',{name:'최종 보정 시작'}));fireEvent.click(screen.getByRole('button',{name:'확정'}));fireEvent.click(screen.getByRole('button',{name:'다음 측정'}));
