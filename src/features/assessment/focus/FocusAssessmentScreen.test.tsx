...
describe('FocusAssessmentScreen', () => {
  it('READY와 4x3 button grid를 렌더하며 label에 정답을 노출하지 않는다', () => {
    ...
    render(<FocusAssessmentScreen onNext={() => undefined} clock={{ now: () => 0 }} frameScheduler={scheduler} createTrialId={() => 'id'} />);
  ...
  it('correct-only 평균과 valid 정답 수를 summary에 표시하고 placeholder로 연결한다', () => {
    let now = 100;
    const scheduler = new ImmediateScheduler(); const onNext = vi.fn();
    render(<FocusAssessmentScreen onNext={onNext} clock={{ now: () => now }} frameScheduler={scheduler} createTrialId={() => String(now)} />);
    ...
    fireEvent.click(screen.getByRole('button', { name: '기본 분석 보기' })); expect(onNext).toHaveBeenCalledOnce();
  });

  it('정답이 없으면 0초 대신 정답 기록 없음을 표시한다', () => {
    let now = 0; const scheduler = new ImmediateScheduler();
    render(<FocusAssessmentScreen onNext={() => undefined} clock={{ now: () => now }} frameScheduler={scheduler} createTrialId={() => String(now)} />);
