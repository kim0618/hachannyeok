...
import { FocusInstrument, InstrumentReadyContent } from '../AssessmentInstruments';
...
export function FocusAssessmentScreen(props: Props) {
  ...
  if (phase === 'running') {
    ...
    return <div className="screen focus-screen focus-running"><header className="progress-header"><span>5 / 5 · 시각 집중</span><span>{progress}</span></header><section className="focus-stage" aria-labelledby="focus-cue"><div className="focus-cue"><p id="focus-cue">이 모양을 찾으세요</p><FocusShape shape={activeConfig.targetShape} /></div><div className={`focus-grid ${visualSkinClass(props.variationSessionId, 'day1_focus_search', trials.length)}`} aria-label="시각 집중 선택지">{activeConfig.items.map((item, index) => <button key={item.id} type="button" className="focus-item" aria-label={`선택지 ${index + 1}`} disabled={!assessment.interactive} onClick={() => assessment.selectItem(item.id)}><FocusShape shape={item.shape} /></button>)}</div></section></div>;
  }
  ...
  return <div className="screen focus-screen ready-screen"><header className="progress-header"><span>시각 집중</span><span>5 / 5</span></header><InstrumentReadyContent instrument={<FocusInstrument />} eyebrow="다섯 번째 측정" titleId="focus-ready-title" title="시각 집중">화면에서 지정된 모양을 최대한 빠르게 찾아 눌러주세요.<br />속도도 보지만, 정확하게 찾는 게 더 중요합니다.</InstrumentReadyContent><div className="bottom-action"><PrimaryButton onClick={assessment.startTrial}>시작하기</PrimaryButton></div></div>;
}
