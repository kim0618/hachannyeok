...
export function TimeAssessmentScreen({ onComplete, clock, dateNow = () => new Date(), createTrialId, baselineSessionDateKey, onDateInvalidated }: TimeAssessmentScreenProps) {
  ...
  return (
    <div className="screen time-screen ready-screen">
      <header className="progress-header"><span>시간 감각</span><span>1 / 3</span></header>
      <section className="ready-content" aria-labelledby="time-ready-title">
        <p className="eyebrow">3초 감각</p>
        <h1 id="time-ready-title">시작하면 시간이<br />표시되지 않습니다.</h1>
        <p>3초라고 느껴지는 순간 바로 눌러주세요.</p>
      </section>
      <div className="bottom-action"><PrimaryButton onClick={assessment.startTrial}>시작하기</PrimaryButton></div>
    </div>
  );
}
